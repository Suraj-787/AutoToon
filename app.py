    ...
    if st.button("Generate Comic") and story and api_key:
        with st.status("Generating Comic...", expanded=True) as status:
            try:
                ...
                # Check if prompts are empty and display errors if they are
                if not panel_prompts or all(p.strip() == "" for p in panel_prompts):
                    st.error("⚠️ No valid panel prompts generated. Please check your input or try again.")
                    st.stop()
